...
The DEFEX compatibility step does not disable DEFEX, patch the kernel, grant zygote access to additional regular `/data` files, or leave a persistent replacement library. It temporarily redirects only ReZygisk's own `libzygisk.so` to a byte-identical tmpfs copy outside `/data`; the persistent original is restored at every terminal path and by interrupted-run recovery.
