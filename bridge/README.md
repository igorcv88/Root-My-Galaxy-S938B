...
Before requesting the official `ksud soft-reboot` lifecycle, the bridge stages `libzygisk.so` in `/dev` and temporarily replaces the module copy with a symlink. Samsung DEFEX resolves the opened file through the symlink, so the zygote no longer performs a root `openat()` against `/data/adb`. The original library inode is restored after verification, while the tmpfs file remains until the next full boot so an injected file-backed mapping is not marked deleted. Broken-symlink recovery restores the original library before a later run. A narrow live-policy addition permits zygote to follow the temporary module-path symlink; it does not grant additional access to `/data` files.

The bridge then applies a boot-scoped instrumentation patch to ReZygisk's existing `post-fs-data.sh`. The patch keeps the official script body but adds a lock and existing-monitor guard before ReZygisk removes `/data/adb/rezygisk`. A repeated post-fs-data invocation during the same kernel boot therefore exits without deleting the first runtime or launching another monitor. The original script is restored only after verification reaches a terminal state. A boot-ID marker prevents an interrupted run from suppressing ReZygisk on a later full boot.
