...
class AutoRootService : Service() {
    ...
    private suspend fun runGate() {
        ...
        try {
            ...
            // BOOT_COMPLETED is already the readiness contract. Only enforce the
            // selected monotonic boot-uptime gate here.
            ...
            // Claim the once-per-boot attempt only when we are actually ready to
            // hand execution off. A killed gate during the stabilization wait no
            // longer consumes the only automatic attempt for that boot.
            ...
            // not prove that :autoroot_exec was created or onServiceConnected ran.
            // Keep the foreground gate visible and fail explicitly if that process
            // never completes the handoff.
    ...
    /** Gate failures are persisted only outside the exploit execution window. */
